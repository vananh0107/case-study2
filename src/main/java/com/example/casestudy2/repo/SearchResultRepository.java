...
public interface SearchResultRepository extends JpaRepository<SearchResult, Integer> {
    ...
    @Query(value = "SELECT  d.run_number,s.search_keywords, s.platform, s.matching_pattern, s.match_keywords, s.remarks, d.search_date,d.img_url,GROUP_CONCAT(rd.result SEPARATOR ', ') AS results " +
            "FROM searchs s " +
